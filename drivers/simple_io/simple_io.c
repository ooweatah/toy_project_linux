...
int toy_simple_io_example_6(void)
{
		...
		int switch_state = gpio_get_value(TOY_GPIO_INPUT);
		
        pr_info("\n 토이 simple io 실험 6\n");
        wait_queue_flag = WAIT_QUEUE_WAIT;
        for (;;) {
				prev_switch_state = switch_state;
				switch_state =  gpio_get_value(TOY_GPIO_INPUT);
				
				 if (prev_switch_state == ON && prev_switch_state != switch_state) {
					 if (ledvalue == ON) {
							ledvalue = OFF;
							pr_info("LEFT LED ON\n");
                			LED_RIGHT_OFF();
